...
/**
 * Engagement rings listing — v2 redesign (design_handoff_mcculloch_product_and_listing).
 * Faceted sidebar with live counts, chips, sort, empty state. Filtering is done
 * client-side over the full catalogue (real products + real /filters facets) so the
 * counts and multi-select behave exactly as the design specifies.
 * Original preserved at pages/EngagementRings.original.tsx.
 */
...
const EngagementRingsV2 = (): JSX.Element => {
  ...
  return (
    <div style={{ background: T.paper, color: T.ink, fontFamily: FONT_BODY, minHeight: "100vh" }}>
      ...
      <div className="erv2" style={{ paddingTop: NAV_H }}>
        {/* Breadcrumb */}
        <div style={{ display: "flex", gap: 10, padding: "18px clamp(24px, 3vw, 52px)", fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "#8A8377" }}>
          <Link to="/">Home</Link><span>/</span><span style={{ color: T.ink }}>Engagement rings</span>
        </div>
        ...
        <section style={{ padding: "clamp(20px,3vw,40px) clamp(24px,3vw,52px) clamp(36px,4vw,56px)", borderBottom: `1px solid ${T.rule}` }}>
          ...
          <p style={{ margin: 0, maxWidth: "42ch", fontSize: 15, lineHeight: 1.75, color: T.body }}>Every ring is made to order in our own workshop. Choose a setting and we will cut and set the stone to your specification.</p>
        </section>
                  ...
                  return (
                    <Link key={p.id} to={`/engagement-rings/${p.slug}`} className="erv2-card" style={{ display: "block" }}>
                      <div style={{ position: "relative", aspectRatio: "4 / 5", background: "#FFFFFF", overflow: "hidden" }}>
                        ...
                        {second && <img src={getMediaUrl(second)} alt="" aria-hidden="true" className="erv2-card-alt" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} loading="lazy" />}
...
export default EngagementRingsV2;
