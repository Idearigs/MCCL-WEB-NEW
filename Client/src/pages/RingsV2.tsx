            ...
            {loading ? (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "clamp(18px,2vw,32px)", marginTop: 32 }} className="rv2-grid">
                {results.map(({ p }) => {
                  ...
                  // Prefer another angle of the same metal colour; fall back to any other image.
                  ...
                  return (
                    <Link key={p.id} to={linkFor(p)} className="rv2-card" style={{ display: "block" }}>
                      <div style={{ position: "relative", aspectRatio: "4 / 5", background: "#FFFFFF", overflow: "hidden" }}>
                        ...
                        {second && <img src={getMediaUrl(second)} alt="" aria-hidden="true" className="rv2-card-alt" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} loading="lazy" />}
