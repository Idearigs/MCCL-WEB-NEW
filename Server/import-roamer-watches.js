...
async function importRoamerWatches() {
  try {
    await sequelize.authenticate();
    console.log('✓ Connected to database\n');

    console.log('═══════════════════════════════════════════');
    console.log('ROAMER WATCHES IMPORT');
    console.log('═══════════════════════════════════════════\n');

    // STEP 1: Create or get Roamer brand
    console.log('STEP 1: Creating/Verifying Roamer Brand\n');

    let [roamerBrand] = await sequelize.query(`
    ...
    if (roamerBrand.length > 0) {
      brandId = roamerBrand[0].id;
      console.log(`✓ Roamer brand already exists (ID: ${brandId})\n`);
    } else {
      const [result] = await sequelize.query(`
        INSERT INTO watch_brands (
          ...
          'Roamer',
          'roamer',
          'Roamer is a renowned Swiss watch brand with over 130 years of heritage, combining traditional craftsmanship with modern design.',
          true,
          0,
          CURRENT_TIMESTAMP,
          CURRENT_TIMESTAMP
      ...
      `);
      brandId = result[0].id;
      console.log(`✓ Created Roamer brand (ID: ${brandId})\n`);
    }
    ...
    for (const collection of collectionsData.collections) {
      ...
      for (const watch of collection.watches) {
        try {
          ...
          successCount++;
          const displayDesc = watch.description ? watch.description.substring(0, 50) : watch.styleNumber;
          console.log(`  ✓ ${watch.styleNumber} - ${displayDesc}...`);
